...
def main():
    ...
    parser.add_argument(
        ...
        "--check_catalogs",
        action="store_true",
        help="Check catalogs, print report and exit.",
    )
    parser.add_argument(
        "-p",
        "--printcat",
    ...
    parser.add_argument(
        ...
        "--rebuild_catalog",
        action="store_true",
        help="Rebuild catalogs and schedules",
    )
    parser.add_argument(
        "-q",
        "--rebuild_sequences",
        action="store_true",
        help="Restarts all sequences - will take effect in next schedule build.",
    )
    parser.add_argument(
        "-a",
        "--scan_sequences",
    ...
    parser.add_argument(
        "-w", "--add_week", action="store_true", help="Add one week to all schedules"
    )
    parser.add_argument(
        "-m", "--add_month", action="store_true", help="Add one month to all schedules"
    )
    parser.add_argument(
        "-d", "--add_day", action="store_true", help="Add one day to all schedules"
    )
    parser.add_argument(
        "-s",
        "--schedule",
    ...
    parser.add_argument(
        ...
        "--delete_schedules",
        action="store_true",
        help="Delete all schedules (but not catalogs)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    if args.graphical_interface or len(sys.argv) <= 1:
        try:
            ...
            logging.getLogger().error(
                "Could not load graphical interface - please install textual"
            )
            logging.getLogger().error(
                "Use this command to install: pip install textual"
            )
            sys.exit(-1)
        ...
        if station_conf["network_type"] == "guide":
            ...
            if station_conf["network_name"] == args.printcat:
                logging.getLogger().info(
                    f"Printing catalog for {station_conf['network_name']}"
                )
            ...
            logging.getLogger().info(
                f"Processing station: {station_conf['network_name']}"
            )
            ...
            if args.check_catalogs:
                ...
                logging.getLogger().info(
                    f"Checking catalog for {station_conf['network_name']}"
                )
                station.check_catalog()
            else:
                logging.getLogger().info(
                    f"Checking for schedule tasks for {station_conf['network_name']}"
                )
                ...
                if args.add_day:
                    ...
                    logging.getLogger().info(
                        "No schedules generated, use -h --help to see available options"
                    )

    if args.printcat and not found_print_target:
        logging.getLogger().error(
            f"Could not find catalog for network named: {args.printcat}"
        )
