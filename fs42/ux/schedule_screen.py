...
class ScheduleScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        for station in StationManager().stations:
            if station["network_type"] != "guide":
                ...
                index += 1
        self.select_station: Select[int] = Select(
            self.options, id="stationselector", prompt="Select Station"
        )
            ...
            case "view_selected":
                ...
                if val == Select.BLANK:
                    ...
                    if schedule is None:
                        ...
                        self.dt.add_columns(
                            "Start Time", "End Time", "Title", "Duration", "Blocks"
                        )
    ...
    def populate_stats(self):
        self.dt.clear(True)
        self.dt.add_columns(
            "Network Name", "Schedule Start Date", "Schedule End Date", "Days"
        )
        ...
        for key in schedules:
            ...
            if _start and _end:
                diff = _end - _start
                self.dt.add_row(
                    key, f"{_start:%Y-%m-%d}", f"{_end:%Y-%m-%d}", f"{diff.days}"
                )
            else:
