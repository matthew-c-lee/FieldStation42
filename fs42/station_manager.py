...
class StationManager(object):
        ...
        if os.path.exists(StationManager.main_config):
            with open(StationManager.main_config) as f:
                try:
                    ...
                    _l.exception(e)
                    _l.error(
                        f"Error loading main config overrides from {StationManager.main_config}"
                    )
                    exit(-1)
    ...
    def load_json_stations(self):
                        ...
                        for to_check in StationManager.filechecks:
                            if to_check in d["station_conf"]:
                                if not os.path.exists(d["station_conf"][to_check]):
                                    _l.error("*" * 60)
                                    _l.error(
                                        f"Error while checking configuration for {fname}"
                                    )
                                    _l.error(
        ...
        self.stations = sorted(
            station_buffer, key=lambda station: station["channel_number"]
        )
