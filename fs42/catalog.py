...
from fs42.catalog_entry import (
    CatalogEntry,
    MatchingContentNotFound,
    NoFillerContentFound,
)
...
class ShowCatalog:
    ...
    def _build_single(self, tag="content"):
        ...
        self._l.info(
            f"Checking for media in {self.config['content_dir']} for single directory"
        )
        ...
        self._l.info(
            f"Building complete - processed {len(self.config['content_dir'])} files"
        )
        self._write_catalog()
    ...
    def _build_standard(self):
        ...
        for fp in start_bumps:
            ...
            if len(sb) == 1:
                ...
                self._l.error(
                    "File paths for start_bump should be relative to the content_dir"
                )
        ...
        for fp in end_bumps:
            ...
            if len(sb) == 1:
                ...
                self._l.error("Start bump specified but not found {fp}")
                self._l.error(
                    "File paths for end_bump should be relative to the content_dir"
                )
        ...
        for tag in self.tags:
            ...
            subdir_clips = MediaProcessor._process_subs(
                tag_dir, tag, bumpdir=(tag == self.config["bump_dir"])
            )
            ...
            if tag == self.config["bump_dir"]:
                (pre, fill, post) = MediaProcessor._by_position(
                    subdir_clips, ShowCatalog.prebump, ShowCatalog.postbump
                )
        ...
        if "sign_off_video" in self.config:
            ...
            self.clip_index["sign_off"] = CatalogEntry(
                self.config["sign_off_video"], video_clip.duration, "sign_off"
            )
        ...
        if "off_air_video" in self.config:
            ...
            self.clip_index["off_air"] = CatalogEntry(
                self.config["off_air_video"], video_clip.duration, "off_air"
            )
        ...
        if "off_air_image" in self.config:
            ...
            self.clip_index["off_air_image"] = CatalogEntry(
                self.config["off_air_image"], MIN_5, "off_air"
            )
                ...
                if "sequence" in slots[k]:
                    ...
                    for seq_tag in to_add:
                        if seq_tag in self.config["clip_shows"]:
                            ...
                            self._l.error(
                                f"{seq_tag} is in the clip shows list, but is declared as a sequence on {day} in slot {k}"
                            )
                            exit(-1)
                        ...
                        if seq_key not in self.sequences:
                            ...
                            file_list = MediaProcessor._rfind_media(
                                f"{self.config['content_dir']}/{seq_tag}"
                            )
    ...
    def load_catalog(self):
        ...
        with open(c_path, "rb") as f:
            try:
                ...
                print(
                    "\033[91m"
                    + "Error loading catalogs - this means you probably need to update your catalog format"
                )
                print(
                    "Please rebuild catalogs by running station_42.py --rebuild_catalog"
                    + "\033[0m"
                )
                sys.exit(-1)
    ...
    def check_catalog(self):
        ...
        if len(too_short):
            print(
                f"{bcolors.FAIL}Found {len(too_short)} videos under 1 second in length:{bcolors.ENDC}"
            )
            for v in too_short:
    ...
    def find_filler(self, seconds, when):
        ...
        if not len(self.clip_index[bump_tag]) and not len(self.clip_index[com_tag]):
            raise NoFillerContentFound(
                "Can't find filler - add commercials and bumps..."
            )
        return self.find_candidate(
            random.choice([bump_tag, com_tag, com_tag]), seconds, when
        )
    ...
    def find_bump(self, seconds, when, position=None):
        ...
        if position:
            if position == ShowCatalog.prebump and len(
                self.clip_index[ShowCatalog.prebump]
            ):
                return self.find_candidate(ShowCatalog.prebump, seconds, when)
            elif position == ShowCatalog.postbump and len(
                self.clip_index[ShowCatalog.postbump]
            ):
