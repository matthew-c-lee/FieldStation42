...
class LiquidSchedule:
    ...
    def _flood(self, start_time, end_target):
        ...
        for i in range(diff.days):
            ...
            block = LiquidLoopBlock(
                content, current_mark, next_mark, self.conf["network_name"]
            )
    ...
    def _fluid(self, start_time, end_target):
        ...
        while current_mark < end_target:
            self._l.debug(
                f"Making schedule for: {current_mark} {current_mark.weekday()} {current_mark.hour}"
            )
            ...
            if tag_str is not None:
                ...
                if tag_str not in self.conf["clip_shows"]:
                    ...
                    if candidate is None:
                        ...
                        next_mark = current_mark + datetime.timedelta(
                    ...
                    if len(clip_content) == 0:
                        ...
                        target_duration = self._calc_target_duration(
                            clip_block.content_duration()
                        )
                        next_mark = current_mark + datetime.timedelta(
                            seconds=target_duration
                        )
                ...
                if candidate is None:
                    self._l.error(
                        f"Schedule logic error: no schedule hints for {current_mark}"
                    )
                    self._l.error(
                        "This indicates that the station is offair, but offair content is not configured"
                    )
                    self._l.error(
                        f"Configure 'off_air_video' or 'off_air_image' for {self.conf['network_name']}"
                    )
                    sys.exit(-1)

                # make it for one hour.
                # TODO: handle when it starts at half hour - just go to next hour (not always one hour)
                next_mark = (current_mark + datetime.timedelta(hours=1)).replace(
                    minute=0, second=0, microsecond=0
                )
                new_block = LiquidOffAirBlock(
                    candidate, current_mark, next_mark, "Offair"
                )
        ...
        match self.conf["network_type"]:
            ...
            case "guide":
                raise NotImplementedError(
                    "Guide schedules are not yet supported for schedules"
                )
