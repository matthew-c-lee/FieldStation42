...
class LiquidClipBlock(LiquidBlock):
    def __init__(
        ...
        if type(content) is list:
            ...
            raise (
                TypeError(
                    f"LiquidClipBlock required content of type list. Got {type(content)} instead"
                )
            )
...
class ReelBlock:
    ...
    def make_plan(self):
        ...
        if self.start_bump is not None:
            entries.append(
                BlockPlanEntry(self.start_bump.path, 0, self.start_bump.duration)
            )
        for comm in self.comms:
            entries.append(BlockPlanEntry(comm.path, 0, comm.duration))
        if self.end_bump is not None:
            entries.append(
                BlockPlanEntry(self.end_bump.path, 0, self.end_bump.duration)
            )
        return entries
