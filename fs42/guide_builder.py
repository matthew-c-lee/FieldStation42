...
class ScheduleQuery:
    ...
    def query_slot(network_name, when):
        ...
        while keep_going:
            programming_block: LiquidBlock = LiquidManager().get_programming_block(
                network_name, current_marker
            )
            ...
            next_marker = (
                current_marker + remaining_duration + datetime.timedelta(seconds=1)
            )
...
class GuideBuilder:
    def __init__(
        self,
        num_blocks=3,
        template_dir="fs42/guide_render/templates/",
        static_dir="fs42/guide_render/static/",
    ):
    ...
    def build_view(self):
        ...
        for station in StationManager().stations:
            ...
            view["meta"].append(
                {"network_name": network_name, "channel_number": channel_number}
            )
