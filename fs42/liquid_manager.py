...
class LiquidManager(object):
            ...
            if _block.start_time > now and hasattr(_block, "sequence_key"):
                ...
                if _block.sequence_key and _block.sequence_key in catalog.sequences:
                    ...
                    if _block.sequence_key not in _reaped:
                        ...
                        # get the sequence
                        _sequence: series.SeriesIndex = catalog.sequences[
                            _block.sequence_key
                        ]
    ...
    def get_extents(self, network_name):
        ...
        if _id not in self.schedules:
            raise (
                ValueError(
                    f"Can't get extent for network named {network_name} - it does not exist."
                )
            )
            ...
            if go_deep:
                ...
                for _entry in _block.plan:
                    next_mark = current_mark + datetime.timedelta(
                        seconds=_entry.duration
                    )
                    print(
                        f"{_entry} start={current_mark.time()} end={next_mark.time()}"
                    )
                    current_mark = next_mark
