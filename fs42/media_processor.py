...
class MediaProcessor:
    ...
    def _process_media(file_list, tag, hints=[]):
        ...
        _l.debug(
            f"_process_media starting processing for tag={tag} on {len(file_list)} files"
        )
        show_clip_list = []
        ...
        for fname in file_list:
            ...
            try:
                ...
                if duration <= 0.0:
                    _l.warning(
                        f"Could not get a duration for tag: {tag}  file: {fname}"
                    )
        ...
        _l.debug(
            f"_process_media completed processing for tag={tag} on {len(file_list)} files"
        )

        if len(failed):
            _l.warning(
                f"Errors were encountered during processing - error count: {len(failed)}"
            )
            count_printed = 0
            for f in failed:
                _l.warning(f)
    ...
    def _get_duration(file_name):
        ...
        if (
            "streams" in probed
            and len(probed["streams"])
            and "duration" in probed["streams"][0]
        ):
    ...
    def _find_media(path):
        ...
        logging.getLogger("MEDIA").debug(
            f"_find_media done scanning {path} {len(file_list)}"
        )
        return file_list
    ...
    def _rfind_media(path):
        ...
        logging.getLogger("MEDIA").debug(
            f"_rfind_media done scanning {path} {len(converted)}"
        )
        return converted
