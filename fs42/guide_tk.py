    ...
    def rotate_message(self):
        ...
        if len(self.conf.images):
            ...
            resized = as_img.resize(
                (
                    int(self.conf.half_w - self.conf.pad * 2),
...
class ScheduleFrame(tk.Frame):
    ...
    def populate_frame(self):
        ...
        lbl_current_time.place(
            x=0, y=0, height=self.conf.sched_h, width=self.conf.network_w
        )
        ...
        for timing in view["timings"]:
            ...
            lbl_time_slot.place(
                x=l_offset, y=0, height=self.conf.sched_h, width=self.conf.sched_w
            )

            l_offset += self.conf.sched_w

        canvas_h = (
            self.conf.sched_h * len(view["rows"])
            + self.conf.footer_height * len(self.conf.footer_messages)
            + 200
        )
        ...
        self.scroll_frame = tk.Frame(
            self.canvas, width=self.conf.width, height=canvas_h, bg=self.conf.bottom_bg
        )
        ...
        for r in range(len(view["rows"])):
            ...
            channel_label.place(
                x=x_offset,
                y=y_offset,
                height=int(self.conf.sched_h),
                width=int(self.conf.network_w),
            )
            ...
            for c in row:
                if c.width > 0:
                    ...
                    the_width = (
                        (self.conf.width - self.conf.network_w) / 5400
                    ) * c.width

                    schedule_label.place(
                        x=x_offset,
                        y=y_offset,
                        height=int(self.conf.sched_h),
                        width=the_width,
        ...
        self.scroll_frame_id = self.canvas.create_window(
        ...
        for msg in self.conf.footer_messages:
            ...
            lbl_footer.place(
                x=self.conf.pad,
                y=y_offset,
                height=self.conf.footer_height,
                width=self.conf.width - self.conf.pad * 2,
            )
